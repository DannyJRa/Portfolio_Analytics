...
# Dynamic infoBoxes
      infoBoxOutput("progressBox")
              ...
              fluidRow(
                ...
                ,
                box(
                  dataTableOutput("table_StartDateDay")
                )
                ######## 
...
##########Save input to rds#########
  ...
  ######################################
    ...
    output$table_StartDateDay <- renderDataTable(
      {
        source("server/plot_StartDateDay.R", local = T)
        #plot output here not in source file; doesnt work
        run2
      })
    ...
    output$plot_basic <- renderLeaflet(
      {
        ...
        plotdata<-Running %>% 
          ...
          select(-c(Day,km,cumsum)) %>% 
          mutate(cadence=100,power=100)

        
        runTr0 <- trackeRdata(plotdata)
        
     
        
        
        
        runs2=c(runTr0,runTr0)
        
        
        plot_basic<-leafletRoute(runTr1,session = 1:2)
        #plot output here not in source file; doesnt work
        plot_basic
      })
